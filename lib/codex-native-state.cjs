...
class CodexNativeState {
  ...
  threadRows() {
    return this.query(`
      SELECT id, rollout_path, created_at, updated_at, source, model,
             ...
             git_branch, agent_path, history_mode, thread_source
      FROM threads
    `);
  }
  ...
  edgeRows() {
    return this.query(`
      SELECT e.parent_thread_id, e.child_thread_id, e.status AS edge_status,
             ...
             c.history_mode AS child_history_mode, c.thread_source AS child_thread_source
  ...
  childFromEdge(row) {
    return this.shapeThread({
      ...
      agent_path: row.child_agent_path, history_mode: row.child_history_mode,
      thread_source: row.child_thread_source,
    });
  }
  ...
  repairReport(query, target, depth = 1) {
    const parents = this.listNativeSubagents(query, target, depth);
    const parentIds = new Set(parents.map(parent => parent.threadId));
    const edges = this.edgeRows().filter(row => parentIds.has(row.parent_thread_id));
    return {
      schema: 'codex-native-child-repair/v1',
      generatedAt: new Date().toISOString(),
      readOnly: true,
      database: this.dbPath,
      controlPlane: {
        service: 'multi_agent_v1',
        status: 'not-observable-from-state-db',
        historicalIdField: 'thread_spawn_edges.child_thread_id',
        nativeControlIdField: 'agent_id',
        note: 'A state row is not proof that the current harness can resume or message the child.',
      },
      parents: parents.map(parent => ({
        ...
        children: parent.children.map(child => ({
          ...
          controlIdCandidate: child.threadId,
          controlStatus: 'unverified',
          edgeStatus: child.edgeStatus,
          depth: child.depth,
          title: child.title,
          name: child.name,
          ...
          model: child.model,
          threadSource: child.threadSource,
          historyMode: child.historyMode,
      ...
      evidence: {
        ...
        missingThreadRows: edges.filter(row => !row.child_title && !row.child_name && !row.child_agent_nickname).map(row => row.child_thread_id),
        limitation: 'The report cannot call multi_agent_v1; verify any controlIdCandidate with the live native control surface.',
      },
    };
  }
}
