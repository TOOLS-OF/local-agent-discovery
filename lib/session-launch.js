...
//              Command: claude --resume <sessionRef> --model <model> [--title <title>]
...
//              [--title <title>] [--dangerously-skip-permissions]
...
class ClaudeSessionLauncher extends SessionLauncher {
  ...
  buildLaunchCommand(params) {
    const action = this._requireString(params, 'action',
      'Must be "resume", "attach", or "fork". ' +
      '"resume" is for dead sessions, "attach" for background daemons (orphaned by pane_close), ' +
      '"fork" for branched copies of running sessions. Using the wrong one silently fails or corrupts.');

    if (!['resume', 'attach', 'fork'].includes(action)) {
      throw new Error(`SessionLauncher: "action" must be "resume", "attach", or "fork", got "${action}".`);
    }
    ...
    // resume or fork
    // NOTE: flag is --name (not --title) in Claude Code v2.1.278+
    // --name sets the display name for picker and terminal title.
